...
Public Class OCMCustomAddProjectSponsorshipFormUIModel
	...
	Private Const GENERAL_PROJECT_SPONSORSHIP As String = "General Project Sponsorship"
...
#End Region
	...
	Private Sub OCMCustomAddProjectSponsorshipFormUIModel_Loaded(ByVal sender As Object, ByVal e As Blackbaud.AppFx.UIModeling.Core.LoadedEventArgs) Handles Me.Loaded
		...
		Me.SPONSORSHIPOPPORTUNITYIDPROJECT.Visible = True
		Me.SPONSORSHIPOPPORTUNITYIDPROJECT.Enabled = True
	...
	Private Sub SetupInitialValues()
		...
		'set the program field to "General Project Sponsorship"
		Me.SPONSORSHIPPROGRAMID.SetValueFromLabel(GENERAL_PROJECT_SPONSORSHIP)
		...
		Else
			'9/26/12 Memphis: need to enable this field since it's a project sponsorship:
	...
	Private Function GetOpportunity() As String
		'9/26/12 Memphis commented out all, and added this line since this is only a Project Sponsorship:
	...
	Private Sub MatchOpportunity()
		...
		'use the MoM Custom datalist:New Guid("c1f482ba-f494-46ae-81bc-db93cc54dc20")
		'default BB datalist: New Guid("156c6b80-a895-411f-bebb-198c7a2f9874")
		request.DataListID = New Guid("c1f482ba-f494-46ae-81bc-db93cc54dc20")
		request.SecurityContext = _securityContext
	...
	Private Sub SetFields()
		...
		'Memphis 9/27/12 changed this to keep the projectid field enabled:
		...
		'Criteria  
		'Memphis added 8/23:
		'Me.CLEARSEARCHACTION.Caption = "Clear"
		'Me.CLEARSEARCHACTION.Enabled = True
		'
	...
	Private Sub _findopportunity_InvokeAction(ByVal sender As Object, ByVal e As UIModeling.Core.InvokeActionEventArgs) Handles _findopportunity.InvokeAction
		...
		Else
			...
			If LockThisChild(Me.MATCHEDOPPORTUNITYID.Value) Then
				...
				'Memphis 9/26/12 added to set the ProjectID that was matched:
				_projectId = Me.MATCHEDOPPORTUNITYID.Value.ToString()
	...
	End Sub
	'Memphis 8/23/12
	Private Sub _clearsearchaction_InvokeAction(ByVal sender As Object, ByVal e As Blackbaud.AppFx.UIModeling.Core.InvokeActionEventArgs) Handles _clearsearchaction.InvokeAction
		ClearMatchedOpportunity()
		Me.FINDOPPORTUNITY.Enabled = True
		Me.CLEARSEARCHACTION.Enabled = False
		'reset the programid field
	...
	Private Sub ClearMatchedOpportunity()
		...
		'Memphis 8/23 reset the program selection:
		Me.SPONSORSHIPPROGRAMID.SetValueFromLabel(GENERAL_PROJECT_SPONSORSHIP)
...
#Region "Payment Tab"
	...
	Private Sub UpdatePaymentFieldsRequired()
		...
		' Memphis 9/12/12 disable the payment fields only if this is a prospect sponsorship
		If _prospectSponsorship = False Then
			If _initialLoadComplete = True Then
	...
	Private Shared Function CreateInstallment(ByVal scheduleDate As Date, ByVal amount As Decimal) As OCMCustomAddProjectSponsorshipFormINSTALLMENTSUIModel
		Dim o As New OCMCustomAddProjectSponsorshipFormINSTALLMENTSUIModel
...
#End Region
	...
	Private Sub UnlockSelectedChildren()
		...
		Dim selectedChildren As List(Of OCMCustomAddProjectSponsorshipFormCHILDRENUIModel) = Me.CHILDREN.Value.ToList()
		For Each child As OCMCustomAddProjectSponsorshipFormCHILDRENUIModel In selectedChildren
	...
	Private Sub ChildrenSelected_ChildInserted(ByVal sender As Object, ByVal e As ValueChangedEventArgs)
		If Not e.NewValue Is Nothing Then
			Dim newChild As OCMCustomAddProjectSponsorshipFormCHILDRENUIModel = DirectCast(e.NewValue, OCMCustomAddProjectSponsorshipFormCHILDRENUIModel)
		End If
	...
	Private Sub _addselectedchild_InvokeAction(ByVal sender As Object, ByVal e As Blackbaud.AppFx.UIModeling.Core.InvokeActionEventArgs) Handles _addselectedchild.InvokeAction
		...
		If Me.SPONSORSHIPOPPORTUNITYIDPROJECT.HasValue Then
			LockThisChild(Me.SPONSORSHIPOPPORTUNITYIDPROJECT.Value)
			AddChildToList(Me.SPONSORSHIPOPPORTUNITYIDPROJECT.Value())
			Me.SPONSORSHIPOPPORTUNITYIDPROJECT.Value = Nothing
			Me.SPONSORSHIPOPPORTUNITYIDPROJECT.UpdateDisplayText(String.Empty)
			'Memphis 8/23
			'Me.CLEARSEARCHACTION.Enabled = False
		End If
	...
	Private Sub AddChildToList()
		...
		If Me.MATCHEDOPPORTUNITYID.HasValue Then
			...
			'Dim newChild As New OCMCustomAddProjectSponsorshipFormCHILDRENUIModel()
	...
	Private Sub AddChildToList(ByVal childId As Guid)
		...
		newChild.ID.Value = Me.SPONSORSHIPPROGRAMID.Value 'New Guid(_projectId)  
		newChild.NAME = New SimpleDataListField(Of Guid) 'Me.SPONSORSHIPOPPORTUNITYIDCHILD
	...
	Private Sub ResetSearchFields()
		Me.SPONSORSHIPPROGRAMID.Value = Nothing
		Me.SPONSORSHIPLOCATIONID.Value = Nothing
		Me.GENDERCODE.Value = Nothing
		Me.ISORPHANEDCODE.Value = Nothing
		Me.ISHIVPOSITIVECODE.Value = Nothing
		Me.HASCONDITIONCODE.Value = Nothing
		Me.SPROPPAGERANGEID.Value = Nothing
	End Sub
	...
	Private Sub _sponsorshipopportunityidproject_SearchItemSelected(ByVal sender As Object, ByVal e As Blackbaud.AppFx.UIModeling.Core.SearchItemSelectedEventArgs) Handles _sponsorshipopportunityidproject.SearchItemSelected
		If Not e.SelectedId = Guid.Empty.ToString() Then
			'turn on the Add Child button
			' Project Search List returns these columns:
			'(0): "448dced9-4160-4f71-8b4c-9b1a67ded89c"
			'(1): "Memphis Sponsorable Project 1"
			'(2): "8-10000145"
			'(3): "Project Sponsorship"
			'(4): "Open"
			'(5): "Available"
			'(6): Nothing
			'(7): "IN Memphis Testing \ MT"
			'
			...
			_lookupid = e.SelectedRow.Values.ToList().Item(2).ToString()
			'set the programid value that the user used for searching for this child:
			_projectId = e.SelectedRow.Values.ToList().Item(0).ToString()
	...
	Private Sub _isprospectsponsorship_ValueChanged(ByVal sender As Object, ByVal e As Blackbaud.AppFx.UIModeling.Core.ValueChangedEventArgs) Handles _isprospectsponsorship.ValueChanged
		' need to change the Interaction Type code table, depending on the sponsorship type:
		If CBool(e.NewValue) Then
			'checked
			_prospectSponsorship = True
			'prospect sponsorship uses USR_PROSPECTPROJECTSPONSORSHIPINTERACTIONTYPECODE
			Me.INTERACTIONTYPECODEID.CodeTableName = "USR_PROSPECTPROJECTSPONSORSHIPINTERACTIONTYPECODE"
		...
		Else
			'unchecked
			_prospectSponsorship = False
			'paid sponsorship uses USR_PAIDPROJECTSPONSORSHIPINTERACTIONTYPECODE
			Me.INTERACTIONTYPECODEID.CodeTableName = "USR_PAIDPROJECTSPONSORSHIPINTERACTIONTYPECODE"
			Me.INTERACTIONTYPECODEID.Value = Nothing
		...
		If e.OldValue <> e.NewValue Then
			...
			'	ValidateSponsorConstituency(Me.SPONSORSHIPCONSTITUENTID.Value)
			'End If
			If Me.REVENUECONSTITUENTID.HasValue AndAlso (Not Me.REVENUECONSTITUENTID.Equals(Guid.Empty)) Then
				ValidateSponsorConstituency(Me.REVENUECONSTITUENTID.Value)
			End If
		End If
